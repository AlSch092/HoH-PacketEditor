...
public:
	...
	PacketWriter(uint32_t h) : m_pos(0), m_buffer(new unsigned char[bufferLen]), m_length(bufferLen) { Write<uint32_t>(h); }
	...
	void WriteString(const std::string &str, int len); // Static-lengthed strings
...
inline
void PacketWriter::WriteString(const std::string &str, int len) {
